...
const OrdersPage = ({ isMobile: propIsMobile, isTablet: propIsTablet }) => {
  ...
  // Fallback to default images for users not in the map
  const getProfileImage = (name) => {
    if (profileImageMap[name]) {
      return profileImageMap[name];
    }
    // For users not in our image map, cycle through available images
    const availableImages = [nataliCraig, kateMorrison, drewCano, orlandoDiggs, andiLane, korayOkumus];
    const index = name.length % availableImages.length;
    return availableImages[index];
  };

  // --- Helpers for initials avatar (kept as fallback) ---
  ...
  // Card View component (uses actual profile images instead of initials)
    ...
    return (
      <Card sx={{
        ...
        <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
          ...
          {/* Avatar + name - NOW USING ACTUAL IMAGES */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
            <Avatar
              src={getProfileImage(order.user.name)}
              sx={{
                width: 24,
                height: 24,
                fontSize: '0.75rem',
                bgcolor: stringToColor(order.user.name), // fallback color if image fails
                opacity: 1,
  ...
  return (
    <Box sx={{ p: isMobile ? 2 : isTablet ? 2.5 : 3, bgcolor: colors.background, minHeight: '100vh' }}>
      ...
      {(isMobile || isTablet) && viewMode === 'card' ? (
        ...
        // Table view - NOW USING ACTUAL IMAGES
